        ...
        for k, v in database.items():
            if k != 'version':
                ...
                for l, w in v.characters.items():
                    ...
                    if (not hasattr(w, 'basePoints')) or 'werecat' in w.basePoints:
                        if 'RAN' not in w.statPoints:
                            w.statPoints['RAN'] = 0
                        w = w.copy()
                        v.characters[w.name.lower()] = w
